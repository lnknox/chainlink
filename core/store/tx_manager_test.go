...
func TestTxManager_CreateTx_RoundRobinSuccess(t *testing.T) {
	...
	ethMock.Context("app.StartAndConnect()", func(ethMock *cltest.EthMock) {
		ethMock.RegisterSubscription("newHeads", newHeads)
	...
	newHeads <- models.BlockHeader{Number: cltest.BigHexInt(sentAt + config.EthGasBumpThreshold())}

	// Attempts will happen eventually because new head is recorded async
	gomega.NewGomegaWithT(t).Eventually(func() []*models.TxAttempt {
...
func TestTxManager_CreateTx_BreakTxAttemptLimit(t *testing.T) {
	...
	newHeads <- models.BlockHeader{Number: cltest.BigHexInt(sentAt + 2*config.EthGasBumpThreshold())}
...
func TestTxManager_LogsETHAndLINKBalancesAfterSuccessfulTx(t *testing.T) {
	...
	confirmedReceipt := models.TxReceipt{
		Hash:        hash,
		BlockNumber: cltest.Int(confirmedHeight),
	}
	require.NoError(t, app.Store.ORM.CreateHead(cltest.Head(1)))
	ethMock.Context("app.StartAndConnect()", func(ethMock *cltest.EthMock) {
		ethMock.RegisterSubscription("newHeads", newHeads)
	...
	ethMock.Context("manager.BumpGasUntilSafe()", func(ethMock *cltest.EthMock) {
		ethMock.Register("eth_getBalance", mockedEthBalance)
		ethMock.Register("eth_call", mockedLinkBalance)
	})
	newHeads <- models.BlockHeader{Number: cltest.BigHexInt(confirmedHeight)}
